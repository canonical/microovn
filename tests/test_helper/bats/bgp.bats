...
bgp_single_peer() {
    ...
    if [ "$autoconfig_bgp" == "yes" ]; then
        ...
        echo "# Manually configuring FRR to start BGP daemon on $bgp_iface (ASN $host_asn)" >&3
        microovn_start_bgp_unnumbered "$MICROOVN_BGP_CONTAINER" "$bgp_iface" "$host_asn" "$vrf_device"
    fi
