...
test_setup_file() {
    echo "tests/test_helper/setup_teardown/$(basename "${1//.bats/.bash}")"
}

# Create test file content
create_test_content() {
    ln -s upgrade.bats "$1"
    ln -s upgrade.bash "$(test_setup_file "$1")"
}
...
# Commit 1
git add "$YAML_FILE" "$UPGRADE_TEST_FILE" "$(test_setup_file "$UPGRADE_TEST_FILE")"
if [ -n "$PREV_LTS_VERSION" ]; then
    git add "$LTS_UPGRADE_TEST_FILE" "$(test_setup_file "$LTS_UPGRADE_TEST_FILE")"
fi
