...
func Start(ctx context.Context, s state.State) error {
	...
	_, err = shared.RunCommandContext(ctx, filepath.Join(paths.Wrappers(), "refresh-expiting-certs"))
	if err != nil {
		logger.Errorf("Failed to execute refresh-expiring-certs script. '%s'", err)
	}
	if err == nil {
		fmt.Println("Certs refreshed")
	}
