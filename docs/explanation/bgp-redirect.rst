...
The last point is important for the purpose of hardware offload. If the BGP
daemon acts as if it was bound to the ``Logical Router Port``, it also
...
This logical port is then bound to one end of a veth pair
(``v<interface_name>-brg``), whilst the second end (``v<interface-name>-bgp``)
is put into the VRF.
...
.. image:: /static/bgp/bgp-multilink.svg
